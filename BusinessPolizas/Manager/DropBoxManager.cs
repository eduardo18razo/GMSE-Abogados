...
namespace Polizas.Business.Manager
{
    public class DropBoxManager
    {
        ...
        public bool CreateFolder(string folder)
        {
            ...
            {
                folder = "/" + folder;
                if (!FolderExists(BusinessVariables.Directorio.CarpetaPrincipalDropBox + folder))
                {
                    var folderArg = new CreateFolderArg(BusinessVariables.Directorio.CarpetaPrincipalDropBox + folder);
                    var folderCreated = DBClient.Files.CreateFolderAsync(folderArg);
                    var result = folderCreated.Result;
                }
                return true;
            }
            catch (Exception ex)
