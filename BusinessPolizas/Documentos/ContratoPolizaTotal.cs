...
{
    public class ContratoPolizaTotal
    {
        public bool GeneraDocumento(int idCliente, int idArrendatario)
        {
            try
            {
                BusinessCliente bCliente = new BusinessCliente();
                BusinessArrendatario bArrendatario = new BusinessArrendatario();
                Cliente cte = bCliente.ObtenerCliente(idCliente);
                Arrendatario arrendatario = bArrendatario.ObtenerArrendatario(idArrendatario);
                if (cte != null && arrendatario != null)
                {
                    string pathSave = BusinessVariables.Directorio.DirectorioAplciacion +
                                      BusinessVariables.Directorio.CarpetaTemporales;
                    using (
                        DocX docX =
                            DocX.Load(string.Format("{0}{1}{2}", BusinessVariables.Directorio.DirectorioAplciacion,
                                BusinessVariables.Directorio.CarpetaPlantillas,
                                BusinessVariables.FileTemplates.PolizaTotal)))
                    {

                        docX.Bookmarks["PersonaFisica"].SetText(cte.PersonaFisica ? "X" : string.Empty);
                        docX.Bookmarks["PersonaFisicaActividadEmpresarial"].SetText(cte.ActividadEmpresarial
                            ? "X"
                            : string.Empty);
                        docX.Bookmarks["PersonaMoral"].SetText(!cte.PersonaFisica ? "X" : string.Empty);
                        docX.Bookmarks["NombreSolicitante"].SetText(cte.Nombre);
                        docX.Bookmarks["RepresentanteLegal"].SetText(!cte.PersonaFisica
                            ? cte.RepresentanteLegal
                            : string.Empty);
                        docX.Bookmarks["RFC"].SetText(cte.Rfc);
                        if (cte.ClienteDireccion.Count > 0)
                        {
                            ClienteDireccion direccion = cte.ClienteDireccion.First();
                            docX.Bookmarks["CalleNumero"].SetText(string.Format("{0} No. Ext{1} No Int{2}",
                        ...
                        string telefonos = cte.ClienteTelefono.Aggregate(string.Empty,
                            (current, telefono) =>
                                current +
                                (telefono.Extensiones.Trim() == string.Empty
                                    ? telefono.Telefono
                                    : string.Format("{0} Ext. {1}", telefono.Telefono, telefono.Extensiones)));
